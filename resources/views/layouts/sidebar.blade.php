...
<aside id="sidebar"
    ...
    x-data="{
                ...
                @foreach ($menuGroup['items'] as $itemIndex => $item)
                    @if (isset($item['subItems']))
                        ...
                        @foreach ($item['subItems'] as $subItem)
                            {{-- CHỈ KIỂM TRA NẾU SUBMENU CÓ PATH (TRÁNH LỖI PHẦN CÓ ACTION) --}}
        ...
        isActive(path) {
            if(!path) return false; {{-- Bảo vệ nếu không có path --}}
                                    ...
                                    @if (isset($item['subItems']))
                                        ...
                                        <div x-show="isSubmenuOpen({{ $groupIndex }}, {{ $itemIndex }}) && ($store.sidebar.isExpanded || $store.sidebar.isHovered || $store.sidebar.isMobileOpen)">
                                            <ul class="mt-2 space-y-1 ml-9">
                                                @foreach ($item['subItems'] as $subItem)
                                                    <li>
                                                        {{-- ĐOẠN XỬ LÝ CLICK EVENT / HOẶC ĐƯỜNG DẪN THƯỜNG Ở ĐÂY --}}
                                                        <a 
                                                            @if(isset($subItem['action']))
                                                                href="#"
                                                                @click.prevent="$dispatch('{{ $subItem['action'] }}')"
